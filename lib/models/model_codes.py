...
class AxialAttention_gated_data(nn.Module):
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=128,
                 stride=1, bias=False, width=False):
        assert (in_planes % groups == 0) and (out_planes % groups == 0)
    ...
    def forward(self, x):
        ...
        sig = F.sigmoid(xn)   # torch.Size([128, 4])
        ...
        # import pdb
        # pdb.set_trace()
        qkv = self.bn_qkv(self.qkv_transform(x))  # torch.Size([128, 32, 128])
        q, k, v = torch.split(qkv.reshape(N * W, self.groups, self.group_planes * 2, H), [self.group_planes // 2, self.group_planes // 2, self.group_planes], dim=2)
        # q: torch.Size([16, 8, 1, 16])
        # Calculate position embedding  torch.Size([4, 64, 64])
        all_embeddings = torch.index_select(self.relative, 1, self.flatten_index).view(self.group_planes * 2, self.kernel_size, self.kernel_size)
        q_embedding, k_embedding, v_embedding = torch.split(all_embeddings, [self.group_planes // 2, self.group_planes // 2, self.group_planes], dim=0)
        qr = torch.einsum('bgci,cij->bgij', q, q_embedding)  # torch.Size([16, 8, 1, 16]) #  1, 32, 32
        kr = torch.einsum('bgci,cij->bgij', k, k_embedding).transpose(2, 3)
        qk = torch.einsum('bgci, bgcj->bgij', q, k)
        # print(qk.shape, qr.shape, kr.shape)
        # import pdb
...
class AxialBlock_gated_data(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv_down(x)  # torch.Size([1, 16, 128, 128])
        out = self.bn1(out)
        out = self.relu(out)

        out = self.hight_block(out)  # torch.Size([1, 16, 128, 128])  # torch.Size([1, 16, 16, 16])
        out = self.width_block(out)  # torch.Size([1, 16, 128, 128])
        out = self.relu(out)

        out = self.conv_up(out)  # torch.Size([1, 32, 128, 128])
        out = self.bn2(out)
...
class mix(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2, zero_init_residual=True,
        ...
        self.relu = nn.ReLU(inplace=True)
        # self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, int(128 * s), layers[0], kernel_size= (img_size//2))  # 128
        self.layer2 = self._make_layer(block, int(256 * s), layers[1], stride=2, kernel_size=(img_size//2),   # 128
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, int(512 * s), layers[2], stride=2, kernel_size=(img_size//4),  # 64
                                       dilate=replace_stride_with_dilation[1])
        # self.layer4 = self._make_layer(block, int(1024 * s), layers[3], stride=2, kernel_size=(img_size//8),
        #                                dilate=replace_stride_with_dilation[2])
        ...
        self.adjust   = nn.Conv2d(int(128*s) , num_classes, kernel_size=1, stride=1, padding=0)
        self.soft     = nn.Softmax(dim=1)
        self.adjust_out1 = nn.Conv2d(64 , num_classes , kernel_size=1, stride=1, padding=0)
        self.adjust_out2 = nn.Conv2d(32 , num_classes , kernel_size=1, stride=1, padding=0)
        self.adjust_out3 = nn.Conv2d(16, num_classes, kernel_size=1, stride=1, padding=0)
        ...
        img_size_p = img_size // 8  # 64

        self.layer1_p = self._make_layer(block, int(128 * s), layers[0], kernel_size= (img_size_p//2))  # 32  16
        self.layer2_p = self._make_layer(block, int(256 * s), layers[1], stride=2, kernel_size=(img_size_p//2),  # 32
                                       dilate=replace_stride_with_dilation[0])
        self.layer3_p = self._make_layer(block, int(512 * s), layers[2], stride=2, kernel_size=(img_size_p//4),  # 16
                                       dilate=replace_stride_with_dilation[1])
        self.layer4_p = self._make_layer(block, int(1024 * s), layers[3], stride=2, kernel_size=(img_size_p//8),  # 8
                                       dilate=replace_stride_with_dilation[2])
    ...
    def _forward_impl(self, x):
        ...
        for i in range(x_size[0]):
            canny[i] = cv2.Canny(im_arr[i], 0.35, 0.6)
        # canny = torch.from_numpy(canny).cuda().float()

        canny = torch.from_numpy(canny).cuda().float()  # 1,256,256
        ...
        # x = self.maxpool(x)
        # pdb.set_trace()
        x1 = self.layer1(x)   # torch.Size([1, 32, 128, 128])
        # print(x1.shape)
        x2 = self.layer2(x1)  # torch.Size([1, 64, 64, 64])
        # print(x2.shape)
        x3 = self.layer3(x2)  # torch.Size([1, 128, 32, 32])
        # # print(x3.shape)
        ...
        x = torch.add(x, x2)  # torch.Size([1, 64, 64, 64])
        out_x2 = F.interpolate(x, scale_factor=(4,4), mode ='bilinear')
        out_x2 = self.adjust_out1(out_x2)
        x = F.relu(F.interpolate(self.decoder4(x2) , scale_factor=(2,2), mode ='bilinear'))
        x = torch.add(x, x1)  # torch.Size([1, 32, 128, 128])
        out_x1 = F.interpolate(x, scale_factor=(2, 2), mode='bilinear')
        out_x1 = self.adjust_out2(out_x1)

        x = F.relu(F.interpolate(self.decoder5(x) , scale_factor=(2,2), mode ='bilinear'))  # torch.Size([1, 16, 256, 256])
        # print(x.shape)
        out_x = self.adjust_out3(x)
        
        # end of full image training 

        # y_out = torch.ones((1,2,128,128))
        x_loc = x.clone()  # torch.Size([1, 16, 256, 256])
        # x = F.relu(F.interpolate(self.decoder5(x) , scale_factor=(2,2), mode ='bilinear'))
        #start
        ...
        for i in range(0,4):
            for j in range(0,4):
                ...
                x_p = self.bn1_p(x_p)
                # x = F.max_pool2d(x,2,2)
                x_p = self.relu(x_p)  # torch.Size([1, 128, 16, 16])
                
                # x = self.maxpool(x)
                # pdb.set_trace()
                x1_p = self.layer1_p(x_p) # torch.Size([1, 32, 16, 16])
                # print(x1.shape)
                x2_p = self.layer2_p(x1_p) # torch.Size([1, 64, 8, 8])
                # print(x2.shape)
                x3_p = self.layer3_p(x2_p)  # torch.Size([1, 128, 4, 4])
                # # print(x3.shape)
                x4_p = self.layer4_p(x3_p)  # torch.Size([1, 256, 2, 2])
                # # print(x4.shape)
                # # pdb.set_trace()
                # # Transposed Convolution Decoder
        ...
        x = torch.add(x,x_loc)  # torch.Size([1, 16, 256, 256])
        x = F.relu(self.decoderf(x))
        
        x = self.adjust(F.relu(x))

        # pdb.set_trace()
        return x,out_x2,out_x1,out_x
...
def mix_net(pretrained=True, **kwargs):
...
def mix_net_gated_d(pretrained=False, **kwargs):
    model = mix(AxialBlock_gated_data, [1, 2, 4, 1], s= 0.125, img_size = 256, imgchan = 3, **kwargs)
    return model
