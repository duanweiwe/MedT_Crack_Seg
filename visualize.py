...
from metrics import LogNLLLoss, classwise_f1
...
parser.add_argument('--val_dataset', default='CRACK500/test', type=str)
...
parser.add_argument('--direc', default='./results-test-crack500', type=str,
                    help='directory to save')
parser.add_argument('--crop', type=int, default=128)
parser.add_argument('--device', default='cuda', type=str)
parser.add_argument('--loaddirec', default='result-crack500-cat/252/MedT.pth', type=str) # load best model
...
dir = os.path.join('tensorboard-crack500-test',datetime.now().strftime("%Y%m%d%H%M%S"))
...
for batch_idx, (X_batch, y_batch, *rest) in enumerate(valloader):
    ...
    im = np.concatenate((yHaT[0, 1, :, :], yval[0, :, :]))
    cv2.imwrite(fulldir + image_filename, im)

    writer.add_image("test_result", np.expand_dims(im , 0), batch_idx+1)
